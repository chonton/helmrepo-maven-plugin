...
public class HelmUpgrade extends HelmRelease {
                ...
                if (nodePort == null) {
                  String warning =
                      String.format(
                          ...
                          selector.getPropertyName(), servicePort.getName());
                  getLog().warn(warning);
                } else {
                  getLog().info("Setting " + selector.getPropertyName() + " to " + nodePort);
                  session
                      .getUserProperties()
                      .setProperty(selector.getPropertyName(), nodePort.toString());
                }
              }
            }
          });
  ...
  private Service findService(
      String releaseName, Map<String, List<Service>> services, PortSelector selector) {
    ...
    if (foundServices == null || foundServices.isEmpty()) {
      String warning =
          String.format(
              ...
              selector.getPropertyName(), releaseName, selector.getServiceName());
      getLog().warn(warning);
      return null;
    }
  ...
  private ServicePort findPort(Service foundService, PortSelector selector) {
    ...
    if (ports == null || ports.isEmpty()) {
      String warning =
          String.format(
              ...
              selector.getPropertyName(), foundService.getMetadata().getName());
      getLog().warn(warning);
      return null;
    }
    ...
    if (portName != null) {
      Optional<ServicePort> optionalPort =
          ports.stream().filter(port -> portName.equals(port.getName())).findFirst();
      if (optionalPort.isEmpty()) {
        String warning =
            String.format(
                "Not setting property %s, no port defined on service %s with portName %s",
                selector.getPropertyName(), foundService.getMetadata().getName(), portName);
        getLog().warn(warning);
        return null;
      }
    ...
    if (ports.size() != 1) {
      String warning =
          String.format(
              ...
              selector.getPropertyName(), foundService.getMetadata().getName(), port.getName());
      getLog().warn(warning);
    }
    return port;
  }
